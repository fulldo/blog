...
`React` 在对应的元素上设置了 `__reactEventHandlers$随机字符串` 对象，即为其 `props` ，如果它有 `onClick` 属性，即可判断其绑定了 `onClick` 事件。
...
const isBoundOnClick = (element, level) => {
  ...
  const key = "__reactEventHandlers$";
...
而我们 B 端产品场景中，大多是应用物料组件库（如 `ant-design`），而每个组件都有唯一的 id 来识别，所以我们只对这些指定的元素组件做上报，如按钮、Tab 选项卡、单选、复选等可点击组件做上报：
